...
\newpage
\begin{defn}
If ROC of the system contains the j$\omega$ axis then the system becomes stable,other wise the system is unstable
\end{defn}
\begin{enumerate}
    \item \textbf{case:1 , $a=3$}\\
    For this value of a, the system is violating the ROC condition so the system is unstable.
    \begin{figure}[htp]
        \centering
        \includegraphics[width=\columnwidth]{unstable.png}
        \caption{ROC plot for $a=3$ }
        \label{fig:my_label}
    \end{figure}
    \item\textbf{case:2 , $a=-2$}\\
   For this value of a, the system is not violating the ROC condition so the system is unstable.
     \begin{figure}[htp]
        \centering
